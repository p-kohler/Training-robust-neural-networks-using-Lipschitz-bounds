    ...
    def train_model(self, train_loader, test_loader, optimizer, criterion, lmbd=None, rho=None, mu=None, parameters=None, c=None, Lip_nom=None):
        ...
        while abs(loss_prev - lossM) >= 0.01:
            ...
            if np.mod(epoch_num, 5) == 0:
                ...
                epoch_CElossM = epoch_CEloss/batch_id
                accuracy = self.test_model(test_loader)
                print('Train Epoch: {}; Loss: {:.6f}; Cross-Entropy Loss: {:.6f}; Lipschitz: {:.3f}; Trivial Lipschitz: {:.3f}; Accuracy: {:.3f}'.format(epoch_num, epoch_lossM, epoch_CElossM, epoch_LipM, L_W, accuracy))
        ...
        return loss_course, CEloss_course, Lip_course, accuracy_course
...
def train_network(model, train_loader, test_loader, lmbd=None, rho=None, mu=None, parameters=None, c=None, Lip_nom=None, T=None, L_des=None):
    ...
    if rho is not None:
        ...
        for i in range(2):
            ...
            t1 = time.time()
            epoch_loss, epoch_CEloss, epoch_Lip, epoch_accuracy = model.train_model(train_loader, test_loader, optimizer, criterion, rho=rho, mu=mu, parameters=parameters)
    ...
    if lmbd is not None:
        loss_course, CEloss_course, Lip_course, accuracy_course = model.train_model(train_loader, test_loader, optimizer, criterion, lmbd)
        parameters = None
    elif c is not None:
        loss_course, CEloss_course, Lip_course, accuracy_course = model.train_model(train_loader, test_loader, optimizer, criterion, c)
        parameters = None
    else:
        loss_course, CEloss_course, Lip_course, accuracy_course = model.train_model(train_loader, test_loader, optimizer, criterion)
        parameters = None
